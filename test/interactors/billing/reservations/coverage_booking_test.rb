...
class Billing::Reservations::CoverageBookingTest < ActiveSupport::TestCase
  ...
  # The wizard's new-card variant (operator/reservations#update_billing_and_
  # create_reservation) books through UpdateBillingAndCreateRoomReservation and
  # now sends enforce_coverage for member bookers. The wiring test above pins
  # the step order; these pin the BEHAVIOR through the card organizer, with the
  # Stripe card-attach step stubbed out.
  def call_with_card(user:, room:, **flags)
    di = (Date.current + 3).to_time.change(hour: 9)
    Billing::Reservations::UpdateBillingAndCreateRoomReservation.call(
      reservation_params: { datetime_in: di, hours: 1.0, minutes: 60, room: room, amenity_ids: [] },
      user: user, location: @loc, token: "tok_visa", out_of_band: false,
      use_bundle_pass: false, use_existing_pass: false, buy_day_pass: false, day_pass_type: nil,
      enforce_coverage: true, **flags)
  end

  test "card path: an uncovered included booking is blocked and rolled back" do
    Billing::Payment::UpdateUserPayment.stubs(:call!)
    ActsAsTenant.with_tenant(@op) do
      u = create(:user, operator: @op, original_location: @loc, current_location: @loc)
      room = included_room
      create(:day_pass_type, operator: @op, location: @loc, included_meeting_room_minutes: 120,
      ...
      r = call_with_card(user: u, room: room)
  ...
  test "card path: a bundle holder books and a pass burns automatically" do
    Billing::Payment::UpdateUserPayment.stubs(:call!)
    ActsAsTenant.with_tenant(@op) do
      u = create(:user, operator: @op, original_location: @loc, current_location: @loc)
      room = included_room
      ...
      r = call_with_card(user: u, room: room) # no flags — the auto path
