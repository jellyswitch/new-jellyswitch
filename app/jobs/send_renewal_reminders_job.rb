    ...
    Operator.find_each do |operator|
      ActsAsTenant.with_tenant(operator) do
        Subscription.where(active: true).find_each do |subscription|
          ...
          begin
            location = subscription.plan&.location

            reminder_days = location&.effective_renewal_reminder_days || operator.renewal_reminder_days || 7
            ...
            UserMailer.renewal_reminder_email(user, operator, subscription, location).deliver_now
